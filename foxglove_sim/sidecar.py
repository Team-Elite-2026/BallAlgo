...
try:
    ...
    _HAS_IMAGE_ANNOTATIONS = False
    _LIDAR_FIELDS = []
    build_live_scene_entities = build_static_scene_entities = None
...
DIRECT_CHANNELS: tuple[DirectChannel, ...] = (
    DirectChannel("/robot/state",   "robot_state",   "stream_pose",     "ballalgo.RobotPoseState"),
    DirectChannel("/ball/state",    "ball_state",    "stream_ball",     "ballalgo.BallPoseState"),
    ...
    DirectChannel("/control/intent", "control_intent", "stream_control", "ballalgo.ControlIntent"),
    DirectChannel("/teensy/raw", "teensy_raw", "stream_control", "ballalgo.TeensyRawTelemetry"),
)
...
class BallAlgoFoxgloveSidecar:
    ...
    def _enrich_snapshot(self, snapshot: dict[str, Any]) -> None:
        ...
        if ball:
            snapshot["ball_state"] = {
                "visible": ball.get("visible", False),
                "field_visible": ball.get("field_visible", False),
                "field_x_mm": ball.get("field_x_mm", 0.0),
                "field_y_mm": ball.get("field_y_mm", 0.0),
                "body_x_m": ball.get("body_x_m", 0.0),
                "body_y_m": ball.get("body_y_m", 0.0),
                "vision_angle_deg": ball.get("vision_angle_deg", 0.0),
                "vision_dist_cal_cm": ball.get("vision_dist_cal", 0.0),
            }
        ...
        if ball and ball.get("visible"):
            snapshot["ball_range"] = {
                ...
                "dist_cal_cm": ball.get("vision_dist_cal", 0.0),
                "body_x_m": ball.get("body_x_m", 0.0),
                "body_y_m": ball.get("body_y_m", 0.0),
            }
