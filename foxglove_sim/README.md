...
## Offline Planner Cases Using The Real C++ Motion Planner

There is now a separate offline harness that uses the real C++ motion planner
instead of the Python fake-path generator.

This tool is intentionally **outside** the production runtime loop. It does not
run the camera, LiDAR, or serial pipeline. Instead, it accepts a manually defined
planner case:

- starting robot pose
- starting ball position
- starting ball field velocity
- chosen goal

and repeatedly calls `MotionPlanner::debugPlan(...)`, then publishes the result
through the same Unix-socket-to-sidecar transport used by the runtime.

That means the Foxglove layout, topic names, and planner panels are the same as
live runtime debugging, but the inputs are fully synthetic and controlled.

Build the tool from the `BallAlgo` root:
...
Example with an explicit custom goal point:

```bash
./build/ballalgo_planner_case \
  --robot-pose 910 1215 25 \
  --ball-position 1180 1540 \
  --ball-velocity-field-mps 0.25 -0.05 \
  --goal-point-centered-cm 35 100
```

Important conventions:

- `--robot-pose` / `--ball-position` use field `mm` with a bottom-left origin
- `--robot-pose-centered-cm` / `--ball-position-centered-cm` use centered `cm`
- velocity inputs are field-frame `m/s`
- `--robot-velocity-field-mps` is optional and defaults to zero
...
- `--publish-hz hz`
- `--duration-s seconds`
- `--config path/to/foxglove.conf`

Because the tool replans on every publish tick, Foxglove clients that connect
after startup will still receive the path shortly after they attach.
